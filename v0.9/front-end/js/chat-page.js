...
$("#chatbox").on("dblclick",".send",function(){
  var text = $(this).find(".text").text();
  var msgID = $(this).find(".id").attr("value");
  localStorage.setItem("replayTo",msgID);
  if(text.length > 25){
    text = text.slice(0,25)+"...";
  }
  $("#replyText").text(text);
  $("#replyTo").text("You:");
  $("#replay").css("height","100px");
});
$("#chatbox").on("dblclick",".receive",function(){
  var text = $(this).find(".text").text();
  var msgID = $(this).find(".id").attr("value");
  localStorage.setItem("replayTo",msgID);
  if(text.length > 25){
  ...
  $("#replyText").text(text);
  $("#replyTo").text(user_name);
  $("#replay").css("height","100px");
});
$("#closeReplay").click(function(){
  localStorage.removeItem("replayTo");
...
function send() {
  ...
  $.ajax({
    ...
    data: {
      ...
      client_id: client_id,
      msg: msg,
      replayTo: (localStorage.getItem("replayTo") != null) ? localStorage.getItem("replayTo") : "0"
    },
    success: (data) => {
      if (data != "success") console.error(data);
      localStorage.removeItem("replayTo");
